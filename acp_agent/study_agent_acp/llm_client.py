...
def build_recommendation_intent_facets_prompt(
    ...
    strict_rules = "\n\n".join(
        [
            ...
            spec,
            "Return exactly ONE JSON object that matches the output schema.",
            "Inside intent_facets, include optional clinical_topic_aliases when the study intent uses an abbreviation, acronym, shorthand, or colloquial clinical phrase that could map to a more standard disease/topic wording.",
            "clinical_topic_aliases must be a short array of strings with at most 5 items; include only exact abbreviation expansions or near-synonymous clinical phrasings of the same main condition/topic.",
            "Do not include broader conditions, narrower complications, procedures, treatments, or speculative related terms in clinical_topic_aliases.",
            "If the topic is already standard and unambiguous, clinical_topic_aliases may be empty.",
            "Examples of good aliases: ADRD -> Alzheimer's disease, Dementia; GI bleed -> Gastrointestinal bleeding, Gastrointestinal hemorrhage; COPD -> Chronic obstructive pulmonary disease.",
            "Do NOT wrap output in markdown, code fences, or prose.",
            "If uncertain, return required keys with empty arrays/strings.",
