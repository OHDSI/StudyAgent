...
from study_agent_core.config import ConfigError, apply_config, load_config, load_secret_environment
...
def main() -> int:
    ...
    parser.add_argument("--config", help="Path to config.yaml. Defaults to config.yaml in the current directory.")
    ...
    try:
        config = load_config(args.config, profile=args.profile, cwd=Path.cwd())
        if config is not None:
            apply_config(config)
        for name, value in load_secret_environment(cwd=Path.cwd()).items():
