...
For a truncated exact match set of at most 500 items, a client can request a fresh complete review session by setting `candidate_limit` to the exact count; it must ask before doing so. For larger sets, request at most 500 returned candidates, narrow the clinical frame, or use Atlas/externally authored concept-set JSON. The R shell labels each lane as returned/ matched/ limit/ truncated and offers `full` or `max500` before it downloads a review CSV. For larger selected policies, the client skill can write an exact `--approval-json` plus a human-readable `--approval-csv` and SHA-256 digest. Approval should identify that immutable policy artifact, not an abbreviated terminal rendering. Its bulk-mark helper uses Python CSV parsing; shell field splitting is not safe for clinical concept names.
