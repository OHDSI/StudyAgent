...
For externally authored sets, `scripts/phenotype_external_concept_set_to_acp.py` deterministically normalizes a single-domain OHDSI Atlas concept-set JSON export, a bare Atlas expression (`{"items": [...]}`), or explicit pasted/file concept IDs into the flow’s `concept_sets` request shape. Atlas policies are preserved; explicit IDs require a caller-supplied concept-set name and OMOP domain. The normalized object remains review input and needs explicit approval before `provided_only` emission.

For a truncated exact match set of at most 100 items, a client can request a fresh complete review session by setting `candidate_limit` to the exact count; it must ask before doing so. For larger selected policies, `phenotype_review_csv_to_concept_sets.py` can write an exact `--approval-json` plus a human-readable `--approval-csv` and SHA-256 digest. Approval should identify that immutable policy artifact, not an abbreviated terminal rendering. `phenotype_review_csv_mark.py` performs user-directed bulk marks using Python CSV parsing; shell field splitting is not safe for clinical concept names.
