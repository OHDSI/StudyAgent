...
sequenceDiagram
    ...
    U->>A: POST /flows/phenotype_recommendation\n{study_intent, top_k, max_results, candidate_limit}
    ...
    M-->>G: overview + spec + output_schema
    G->>L: final recommendation prompt\n(study_intent + final compact candidates)
    ...
    G->>G: build deterministic final payload\n(ids come from enforced shortlist;\nLLM supplies justifications where usable)
    G->>C: phenotype_recommendations(...)
    C-->>G: final grounded response

    G-->>A: {status, search, intent_facets, planning,\nrecommendations, diagnostics}
    A-->>U: HTTP 200 JSON
```
...
flowchart LR
    ...
    subgraph ACP
        ...
        A2[Run phenotype_recommendation_flow]
        A3[Intent facet extraction]
        A4[Planning rerank + shortlist enforcement]
    ...
    subgraph Core
        C1[phenotype_recommendation_plan validator]
        C2[phenotype_recommendations validator]
    end
